...
export type PendingRequest = {
  id: string; visitor_name: string; visitor_type: string; relationship: string | null; visit_date: string;
...
export function useApprovals() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: e } = await supabase.from(T.requests)
      .select('id, visitor_name, visitor_type, relationship, visit_date, time_window, purpose, requester:profiles!requester_id(full_name)')
